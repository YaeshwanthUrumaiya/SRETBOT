...
let isBotTyping = 0; //0 is typing, 1 is has answered. used in lines 71 and 509

// var handoff = false; UNUSED Human handoff

function myFunction() { //use this to trigger diff animations
  if(isBotTyping==0){
...
function redirectgifs() {
  var gif = document.getElementById('gif');
  var gif2 = document.getElementById('gif2');
  var overlayGif = document.getElementById('overlaygif');

  window.setTimeout(function() {
    gif.src = 'assets/img/StaticScreen.gif';
  }, 3750);
...
var observer = new IntersectionObserver(function(entries) {
  entries.forEach(function(entry) {
    if (entry.isIntersecting) {
      // The div is currently on the screen
      isDivOnScreen = true;
...
function setBotResponse(response) {
  ...
  setTimeout(() => {
    ...
    if (response.length < 1) { // There is no response from Rasa or human
      const fallbackMsg = "I don't understand, can you please try again?";
      const BotResponse = `
